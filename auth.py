...
from models import User, MembershipRequest, db
from utils import check_validate_input, get_hashed_password, check_password
...
def getAuthCodeHash(ip: str, savedCode: str) -> str:
    ...
    if len(parts) != 2:
        return savedCode
    return bcrypt.hashpw(parts[0] + ip, b64decode(parts[1]))
...
def login_post():
    ...
    if not valiteIPAdress(ip):
        ...
        return redirect(url_for('auth.login'))
    salt = bcrypt.gensalt()
    encoded = b64encode(salt.encode()).decode()
    authCode = bcrypt.hashpw(newAuthCode + ip, salt)
    next = "2fa"
    if user.secret == "":
        next = "2fasetup"
